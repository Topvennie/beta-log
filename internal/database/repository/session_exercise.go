...
func NewSessionExercise() *SessionExercise {
	return &SessionExercise{}
}

func (s *SessionExercise) GetBySession(ctx context.Context, sessionID int) ([]*model.SessionExercise, error) {
	sessionExercises, err := queries(ctx).SessionExerciseGetBySession(ctx, int32(sessionID))
	if err != nil {
...
func (s *SessionExercise) Create(ctx context.Context, sessionExercise *model.SessionExercise) error {
	id, err := queries(ctx).SessionExerciseCreate(ctx, sqlc.SessionExerciseCreateParams{
...
func (s *SessionExercise) Delete(ctx context.Context, id int) error {
	if err := queries(ctx).SessionExerciseDelete(ctx, int32(id)); err != nil {
...
func (s *SessionExercise) DeleteBySession(ctx context.Context, sessionID int) error {
	if err := queries(ctx).SessionExerciseDeleteBySession(ctx, int32(sessionID)); err != nil {
