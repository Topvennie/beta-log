...
const formatDuration = (nanos: number) => {
  ...
  return <span>{sString}<span className="text-neutral-400">{msString}</span></span>
}
  ...
  return (
    <Table
      columns={[
        ...
        { accessor: "runAt", render: ({ runAt }) => <p>{formatDate(runAt)}</p> },
        { accessor: "duration", render: ({ duration }) => <p>{formatDuration(duration)}</p> },
        {
          accessor: "result",
          title: "Result",
